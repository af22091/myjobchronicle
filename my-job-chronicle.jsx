  ...
  return (
    <div style={{ fontFamily: T.bf, background: T.bg, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", color: T.t1 }}>
      ...
      <div style={{ background: T.card, border: "1px solid " + T.bor, borderRadius: 20, padding: 32, width: "100%", maxWidth: 380, zIndex: 10, boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }}>
        ...
        <p style={{ fontFamily: T.mf, fontSize: 11, color: T.t3, textAlign: "center", marginBottom: 8 }}>// Please login to continue</p>
        <p style={{ fontSize: 10, color: T.t3, textAlign: "center", marginBottom: 24 }}>※ 機能をお試しになりたい方は、ユーザー名に「demo」と入力してログインしてください</p>
...
function JobChronicleApp({ currentUser, handleLogout }){
  ...
  const[cos,setCos]=useState(() => {
    ...
    return saved ? JSON.parse(saved) : (currentUser === "demo" ? COMPANIES : []);
  });
  const[evs,setEvs]=useState(() => {
    const saved = localStorage.getItem("job_evs_" + currentUser);
    return saved ? JSON.parse(saved) : (currentUser === "demo" ? EVENTS : []);
  });

  const[syncId,setSyncId]=useState(() => localStorage.getItem("job_syncId_" + currentUser) || "");
  ...
  useEffect(() => {
    localStorage.setItem("job_syncId_" + currentUser, syncId);
  }, [syncId, currentUser]);
  ...
  function exportData() {
    const data = { cos: cos, evs: evs };
    const blob = new Blob([JSON.stringify(data)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "my-job-chronicle-" + currentUser + ".json";
    a.click();
  ...
  async function syncFromCloud() {
    const code = prompt("クラウドから復元します。\n同期コードを入力してください：", syncId);
    if (!code) return;
    try {
      ...
      if (res.ok) {
        ...
        alert("同期コードが見つからないか、エラーが発生しました。");
      }
    } catch(err) {
      alert("復元に失敗しました。");
    }
  ...
  function cname(id){var c=cos.find(function(c){return c.id===parseInt(id);});return c?c.name:(id||"不明");}
  ...
  function saveE(){if(!eForm.title||!eForm.dateStart)return;var t=eForm.companyName!==undefined?eForm.companyName:eForm.companyId;var m=cos.find(function(c){return c.name===t||c.id===parseInt(t);});var f=Object.assign({},eForm,{companyId:m?m.id:(t||"")});delete f.companyName;if(!f.dateEnd)f.dateEnd=f.dateStart;f.isMultiDay=f.dateEnd!==f.dateStart;if(editE){setEvs(function(es){return es.map(function(e){return e.id===editE.id?Object.assign({},e,f):e;});});}else{setEvs(function(es){return es.concat([Object.assign({id:nid(es)},f)]);});}setEMod(false);}
  ...
  return(
    <div style={{fontFamily:T.bf,background:T.bg,minHeight:"100vh",color:T.t1}}>
      ...
      <div style={{position:"sticky",top:0,zIndex:100,background:T.hbg,backdropFilter:"blur(20px)",borderBottom:"1px solid "+T.bor}}>
        <div style={{maxWidth:1280,margin:"0 auto",padding:"0 22px",display:"flex",alignItems:"center",gap:2}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <button onClick={syncFromCloud} className="ib" style={{background:T.sur,border:"1px solid "+T.bor,borderRadius:6,padding:"4px 10px",fontSize:11,color:T.t2,fontFamily:T.bf}}>☁ 復元</button>
            <button onClick={syncToCloud} className="ib" style={{background:T.sur,border:"1px solid "+T.bor,borderRadius:6,padding:"4px 10px",fontSize:11,color:T.t2,fontFamily:T.bf}}>☁ 保存</button>
            <button onClick={function(){document.getElementById('import-json').click();}} className="ib" style={{background:T.sur,border:"1px solid "+T.bor,borderRadius:6,padding:"4px 10px",fontSize:11,color:T.t2,fontFamily:T.bf}}>インポート</button>
            <button onClick={exportData} className="ib" style={{background:T.sur,border:"1px solid "+T.bor,borderRadius:6,padding:"4px 10px",fontSize:11,color:T.t2,fontFamily:T.bf}}>エクスポート</button>
                  ...
                  return(
                    <button key={id} onClick={function(){setTid(id);setTOpen(false);}} className="ib"
                      ...
                      <div style={{textAlign:"left"}}>
                        <div style={{fontSize:12,fontWeight:700,color:T.t1,fontFamily:T.bf}}>{id==="dark"?"DARK":"LIGHT"}</div>
        ...
        {tab==="home"&&(
          <div>
            ...
            <div className="fu1" style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10,marginBottom:20}}>
              {[{l:"登録企業",v:cos.length,c:T.acc,act:function(){setTab("companies");setFSt("ALL");}},{l:"選考中",v:cos.filter(function(c){return["ES提出済","選考中","最終面接"].indexOf(c.status)>=0;}).length,c:T.pur,act:function(){setTab("companies");setFSt("選考中");}},{l:"内定",v:cos.filter(function(c){return c.status==="内定";}).length,c:T.grn,act:function(){setTab("companies");setFSt("内定");}},{l:"ES締切",v:evs.filter(function(e){return e.type==="ES締切"&&e.dateStart>=td;}).length,c:T.red,act:function(){setTab("calendar");}},{l:"直近7日",v:upcoming.filter(function(e){var d=dUntil(e.dateStart);return d!==null&&d<=7;}).length,c:T.gold,act:function(){setTab("calendar");}}].map(function(s){return(
                <div key={s.l} onClick={s.act} className="ch" style={{background:T.card,border:"1px solid "+T.bor,borderRadius:12,padding:"16px 14px",cursor:"pointer"}}>
            ...
            <div className="fu1" style={{display:"grid",gridTemplateColumns:"1fr 280px",gap:14}}>
              <div style={card}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                  ...
                  <span style={{fontFamily:T.bf,fontWeight:700,fontSize:17,color:T.t1}}>{calY}年 {MO[calM]}</span>
      ...
      <Modal show={eMod} onClose={function(){setEMod(false);}} title={editE?"イベントを編集":"イベントを追加"} T={T}>
        <input list="company-list" value={eForm.companyName!==undefined?eForm.companyName:cname(eForm.companyId)} onChange={function(e){setEForm(function(f){return Object.assign({},f,{companyName:e.target.value});});}} placeholder="関連企業・主催者 (入力または選択)" style={S}/>
        <datalist id="company-list">{cos.map(function(c){return <option key={c.id} value={c.name}/>;})}</datalist>
        <input list="etypes-list" value={eForm.type} onChange={function(e){setEForm(function(f){return Object.assign({},f,{type:e.target.value});});}} placeholder="イベント種別 (入力または選択)" style={S}/>
        <datalist id="etypes-list">{ETYPES.map(function(t){return <option key={t} value={t}/>;})}</datalist>
